      ...
      <div>
        <template v-if="searchResults.length > 0">
          ...
          <form @submit.prevent="addPerson" v-if="searchResults.length > 0 && selectedPerson">
            ...
            <label for="newAvance">Наименование:</label>
            <select id="tourSelect" v-model="selectedMoto">
              <option v-for="(equipment_name, index) in motos" :key="index" :value="equipment_name">{{ equipment_name }}
              </option>
            </select>
  ...
  try {
    ...
    async fetchBikes() {
      try {
        ...
        this.motos = response.data.map(moto => moto.equipment_name);
